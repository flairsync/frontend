...
export default function PairingScreen({ stationType, onLinked }: Props) {
  ...
  async function handleLink() {
    ...
    try {
      ...
      setError(e.response?.data?.message ?? t("pairing_screen.errors.invalid_code"));
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="flex-1 flex flex-col items-center justify-center p-6 bg-slate-950 min-h-screen">
      <div className="w-full max-w-sm flex flex-col gap-8">
        ...
        <div className="text-center">
          ...
          <h1 className="text-2xl font-black text-slate-100 tracking-tight">{t("pairing_screen.title")}</h1>
        ...
        <Card className="bg-slate-900 border-slate-800">
          <CardContent className="p-6 flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Input
                placeholder={t("pairing_screen.form.pairing_code_placeholder")}
                value={code}
            ...
            >
              {loading ? (
                <>
                  ...
                  {t("pairing_screen.form.linking")}
                </>
              ) : (
                <>
                  <Unplug className="w-4 h-4" />
                  {t("pairing_screen.form.link_device")}
                </>
              )}
            </Button>
          </CardContent>
