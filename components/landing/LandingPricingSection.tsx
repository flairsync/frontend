...
const LandingPricingSection = () => {
    ...
    useLayoutEffect(() => {
        scope.current = createScope().add(() => {
            utils.$(["#landing_pricing_cards_container", "#landing_pricing_selector"]).forEach(($el) => {
                animate($el, {
                    opacity: [0, 1],
                    y: ["10rem", "0rem"],
                    duration: 3000,
    ...
    return (
        <div className="min-h-screen bg-zinc-50 font-sans text-zinc-900 flex flex-col items-center justify-center p-8">
            ...
            {/* Pricing Cards */}
            <div id='landing_pricing_cards_container' className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
                            ...
                            <ul className="space-y-3 mb-8">
                                {plan.features.map((feature, i) => (
                                    <li key={i} className="flex items-center text-sm font-medium">
                                        <Check className={cn("w-4 h-4 mr-3", plan.name === 'Starter' ? 'text-white' : 'text-[#6366F1]')} />
                                        {feature}
                                    </li>
                                ))}
                            </ul>
