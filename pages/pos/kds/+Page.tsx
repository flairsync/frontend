...
import { 
  ...
  Trash2, RotateCcw, Utensils, Package, Search 
} from 'lucide-react';
...
const MOCK_KDS_ORDERS: KdsOrder[] = [
  ...
  },
  {
    ...
    status: 'pending',
    items: [
      { id: 'it5', name: 'Pepperoni Pizza', quantity: 1 },
    ]
  }
];
...
export default function KdsPage() {
  ...
  const toggleItemCompletion = (orderId: string, itemId: string) => {
    setOrders(prev => prev.map(o => {
        ...
        return {
            ...o,
            items: o.items.map(it => it.id === itemId ? { ...it, isCompleted: !it.isCompleted } : it)
        };
    }));
  };
    ...
    <div className="h-screen flex flex-col bg-slate-950 text-slate-100 overflow-hidden font-sans antialiased">
      ...
      <ScrollArea className="flex-1 bg-slate-950">
        <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
          {orders.filter(o => o.status !== 'ready' || true).map(order => (
            <Card 
                key={order.id} 
                className={`flex flex-col h-[480px] border-2 bg-slate-900 shadow-2xl transition-all ${
                    order.status === 'ready' ? 'border-green-500/30 opacity-60' : 
                    order.status === 'preparing' ? 'border-primary/40' : 'border-slate-800'
                }`}
            >
              {/* Ticket Header */}
              <div className={`p-4 flex flex-col gap-2 ${
                  order.status === 'ready' ? 'bg-green-500/10' : 
                  order.status === 'preparing' ? 'bg-primary/5' : 'bg-slate-950/20'
              }`}>
                <div className="flex justify-between items-start">
                    <div className="flex items-center gap-2">
                        <div className={`p-1.5 rounded-lg ${order.mode === 'dine-in' ? 'bg-primary/20 text-primary' : 'bg-amber-500/20 text-amber-500'}`}>
                            {order.mode === 'dine-in' ? <Utensils className="w-3.5 h-3.5" /> : <Package className="w-3.5 h-3.5" />}
                        </div>
                        <span className="text-2xl font-black text-white">{order.tableName || `#${order.orderNumber}`}</span>
                    </div>
