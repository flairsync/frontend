...
const baseUrl = `${import.meta.env.VITE_API_BASE_URL}/shifts`;
...
export const buildShiftExportUrl = (
  ...
  if (employmentId) params.append('employmentId', employmentId);
  return `${import.meta.env.VITE_API_BASE_URL}/shifts/export?${params.toString()}`;
};
...
export const checkInApiCall = (data: CheckInDto) => {
  return flairapi.post(`${import.meta.env.VITE_API_BASE_URL}/attendance/check-in`, data);
};

export const checkOutApiCall = (data: CheckOutDto) => {
  return flairapi.post(`${import.meta.env.VITE_API_BASE_URL}/attendance/check-out`, data);
};

export const startBreakApiCall = (data: CheckOutDto) => {
  return flairapi.post(`${import.meta.env.VITE_API_BASE_URL}/attendance/break/start`, data);
};

export const endBreakApiCall = (data: CheckOutDto) => {
  return flairapi.post(`${import.meta.env.VITE_API_BASE_URL}/attendance/break/end`, data);
};
...
export const fetchAttendanceLogsApiCall = async (
  ...
  return unwrapPaginated(await flairapi.get(`${import.meta.env.VITE_API_BASE_URL}/attendance/business/${businessId}${qs ? `?${qs}` : ''}`));
};

export const fetchAttendanceByIdApiCall = async (id: string) =>
  unwrap(await flairapi.get(`${import.meta.env.VITE_API_BASE_URL}/attendance/${id}`));
...
export const fetchMyAttendanceApiCall = async (
  ...
  if (filters?.page) params.append('page', String(filters.page));
  if (filters?.limit) params.append('limit', String(filters.limit));
  return unwrapPaginated(await flairapi.get(`${import.meta.env.VITE_API_BASE_URL}/attendance/me?${params.toString()}`));
};

export const fetchTodayAttendanceDashboardApiCall = async (businessId: string, date?: string) => {
  const url = `${import.meta.env.VITE_API_BASE_URL}/attendance/today?businessId=${businessId}`;
...
export const validateAttendanceApiCall = (attendanceId: string, data: ValidateAttendanceDto) => {
  return flairapi.post(`${import.meta.env.VITE_API_BASE_URL}/attendance/${attendanceId}/validate`, data);
};
...
export const fetchAttendanceSummaryApiCall = async (businessId: string, startDate: string, endDate: string, employmentId?: string) => {
  const params = new URLSearchParams({ businessId, startDate, endDate });
  if (employmentId) params.append('employmentId', employmentId);
  return unwrap(await flairapi.get(`${import.meta.env.VITE_API_BASE_URL}/attendance/summary?${params.toString()}`));
};

// Absence Records API
const absenceBaseUrl = `${import.meta.env.VITE_API_BASE_URL}/attendance/absences`;
