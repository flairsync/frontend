...
const getReservationsUrl = (businessId: string) => {
    return `${import.meta.env.PUBLIC_ENV__BASE_URL}/businesses/${businessId}/reservations`;
};
...
export const lookupUserApiCall = async (email?: string, phone?: string) =>
    unwrap(await flairapi.get(`${import.meta.env.PUBLIC_ENV__BASE_URL}/users/lookup`, { params: { email, phone } }));
